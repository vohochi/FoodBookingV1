...
export const getAllVouchers = async (
  ...
  try {
    const response = await fetchData<{ total: number; vouchers: Voucher[] }>(
...
export const createVoucher = async (voucher: Voucher) => {
  ...
  export const updateVoucher = async (id: string, voucher: Voucher) => {
    try {
      ...
      if (voucher.limit !== undefined) formData.append('limit', voucher.limit.toString()); // Convert number to string
  ...
  export const deleteVoucher = async (id: string): Promise<void> => {
    try {
      await deleteData(`/api/admin/vouchers/${id}`);
      console.log(`voucher with id ${id} deleted.`);
    } catch (error) {
